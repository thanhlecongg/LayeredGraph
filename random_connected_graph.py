# Created by Thanh C. Le at 2019-12-15
...
def create_graph(num_nodes, num_edges, loops=False,multigraph=False,digraph=True):
    try:
        nodes = [x for x in range(int(num_nodes))]
    except ValueError:
        raise TypeError('nodes argument must be a filename or an integer')

    graph = random_walk(nodes, num_edges, loops, multigraph,
                     digraph)
    G = nx.Graph()
